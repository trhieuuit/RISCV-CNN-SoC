...
module control_unit(
    ...
    output reg  [4:0] alu_op_o,            // ALU op selection
    
    output reg        reg_we_en_o,     // Register File Write enable
    output reg  [1:0] wb_sel_o,           // Register File Write back MUX

    output reg        is_jalr_o,          // Recognize jalr
    output reg  [2:0] bju_op_o,      // Recognize Branch instructions
    ...
    output reg        dmem_we_o,          // DMEM write
    output reg        is_load_o,        // DMEM read
    
    output reg        done_o             // Recognize Ecall
);
...
//==========================================================//
        case(opcode_i)
            `R_TYPE_OPCODE: begin  
                reg_we_en_o = `REG_WRITE_EN_1;
                op1sel_o       = `DATA1;
                op2sel_o       = `DATA2;
                wb_sel_o       = `ALU;
                alu_op_o        = {funct3_i, funct7_i[5], funct7_i[0]};
            end
...
//==========================================================//
            `I_TYPE_OPCODE: begin
                ...
                if (funct3_i == 3'b001 || funct3_i == 3'b101) begin
                    alu_op_o    = {funct3_i, funct7_i[5], 1'b0};
                end else begin
                    alu_op_o    = {funct3_i, 2'b00};
                end
            end
